...
export function fromHexAndDirection(
  ...
  switch (direction) {
    ...
    case HexDirection.E:
      return createCornerCoordinates(getNeighbor(hex, HexDirection.SE), CornerPosition.North);
    case HexDirection.NE:
      return createCornerCoordinates(getNeighbor(hex, HexDirection.NE), CornerPosition.South);
    case HexDirection.NW:
      return createCornerCoordinates(hex, CornerPosition.North);
    case HexDirection.W:
      return createCornerCoordinates(getNeighbor(hex, HexDirection.NW), CornerPosition.South);
    case HexDirection.SW:
      return createCornerCoordinates(getNeighbor(hex, HexDirection.SW), CornerPosition.North);
  }
}
...
export function toHexAndDirection(corner: CornerCoordinates): {
  ...
  const direction = corner.position === CornerPosition.North ? HexDirection.NE : HexDirection.SE;
  return { hex: corner.hex, direction };
}

export function cornerToWorldPoint(corner: CornerCoordinates): [number, number, number] {
  const worldPoint = toWorldPoint(corner.hex);
  return [
    worldPoint[0],
    worldPoint[1],
    worldPoint[2] + (corner.position === CornerPosition.North ? -1 : 1) * HexMetrics.outerRadius,
  ];
}
...
export function cornersEqual(a: CornerCoordinates, b: CornerCoordinates): boolean {
  return a.hex.x === b.hex.x && a.hex.z === b.hex.z && a.position === b.position;
}

/**
 * Returns true if the two corners are adjacent, false otherwise.
 */
export function cornersAdjacent(a: CornerCoordinates, b: CornerCoordinates): boolean {
  if (a.position === CornerPosition.North) {
    const adjacentCorners = [
      // Towards SW
      createCornerCoordinates(getNeighbor(a.hex, HexDirection.NW), CornerPosition.South),
      // Towards SE
      createCornerCoordinates(getNeighbor(a.hex, HexDirection.NE), CornerPosition.South),
      // Towards N
    ...
    const adjacentCorners = [
      // Towards NW
      createCornerCoordinates(getNeighbor(a.hex, HexDirection.SW), CornerPosition.North),
      // Towards NE
      createCornerCoordinates(getNeighbor(a.hex, HexDirection.SE), CornerPosition.North),
      // Towards S
      createCornerCoordinates(
